...
export default function Home(){
  ...
  return <div className="v20-home">
    <section className="v20-hero">
      ...
      <div className="v20-hero-art"><img className="v20-cart" src={asset("/assets/cart.webp")} alt="Illustrated Thindi Angdi snack cart"/><span className="v20-seal">ಹೃದಯದಿಂದ<br/><b>ನಿಮ್ಮ ಮನೆಗೆ</b></span></div>
    ...
    <section className="v20-map-section v20-section" id="places"><header className="v20-heading light"><div><span className="v20-kicker">A CARTOGRAPHY OF TASTE</span><h2>Tap Karnataka.<br/><em>Meet its thindi.</em></h2></div><p>Ten places. Ten ways into the shelf.</p></header>
      <nav className="v20-place-index" aria-label="Jump to a place">{places.map((p,i)=><button key={p.slug} type="button" className={active===i?"active":""} onClick={()=>setActive(active===i?null:i)}><i>{String(i+1).padStart(2,"0")}</i>{p.name}</button>)}</nav>
